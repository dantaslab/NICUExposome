...
SM_DF <- read.delim('./Desktop/Projects/NEC/final_analysis/RNA/taxonomy/GLMM_prescreen_SD/211108_RNA_metaphlan_matrix_SD.txt', header = TRUE, row.names = 1)
metadata<-read.delim('Desktop/Projects/NEC/final_analysis/RNA/taxonomy/GLMM_prescreen_SD/210920_NECmetadata_lifetime.txt', header = TRUE)
...
write.table(shannDivMetaData, './Desktop/Projects/NEC/final_analysis/RNA/taxonomy/GLMM_prescreen_SD/210921_SD_Meta_lifetime.txt', sep='\t')


GLMM_SD_acute<-read.delim("./Desktop/Projects/NEC/final_analysis/RNA/taxonomy/GLMM_prescreen_SD/210921_SD_Meta_acute.txt", header = TRUE,check.names = FALSE, row.names=1)
...
FIRSTPASSMODEL.glm <- lmer(ShannonDiversity~ (1|Patient) +DOL_log + GA_birth_log+Birthweight_log+
...
#2C. inspect model
BACKFITTEDMODEL.glm <-lmer(ShannonDiversity~ (1|Patient)+(0 + DOL_log | Patient) +DOL_log+GA_birth_log+ABX_recent+Recent_penicillin+Recent_carbapenem+Recent_3rd_gen_cephalosporin+Recent_4th_gen_cephalosporin+
...
GLMM_SD_lifetime<-read.delim("./Desktop/Projects/NEC/final_analysis/RNA/taxonomy/GLMM_prescreen_SD/210921_SD_Meta_lifetime.txt", header = TRUE,check.names = FALSE, row.names=1)
...
#2A. Make First Past Model
FIRSTPASSMODEL_lifetime.glm <- lmer(ShannonDiversity~ (1|Patient) +DOL_log + GA_birth_log+Birthweight_log+
                             ...
                             route+small_for_GA+hifi+pda+inf+chronic_lung_dis+intraventricular_hemorrhage+milk_cumulative_log+formula_cumulative_log,
                           REML=FALSE,
...
#2C. inspect model
BACKFITTEDMODEL_lifetime.glm <-lmer(ShannonDiversity~ (1|Patient)+(0 + DOL_log | Patient) +DOL_log+GA_birth_log+Abx_cumulative_log+ANTIM_cumulative_log+mat_ppbmi_log+milk_cumulative_log,
                           REML=FALSE,
                           data = GLMM_SD_lifetime)
